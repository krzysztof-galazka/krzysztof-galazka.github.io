...
var datatableRowTemplate = $('[data-datatable-rowtemplate]').
children()[0];
...
element.find('[data-task-name-section] [data-task-nameparagraph]').
text(data.title);
element.find('[data-task-name-section] [data-task-nameinput]').
val(data.title);
element.find('[data-task-content-section] [data-task-contentparagraph]').
text(data.content);
element.find('[data-task-content-section] [data-task-contentinput]').
val(data.content);
return element;
}
...
parentEl.find('[data-task-contentparagraph]').
text(taskContent);
}
});
}
...
var taskContent = parentEl.find('[data-task-contentparagraph]').
text();
parentEl.find('[data-task-name-input]').val(taskTitle);
parentEl.find('[data-task-content-input]').val(taskContent);
}
$('[data-task-add-form]').on('submit', handleTaskSubmitRequest);
tasksContainer.on('click','[data-task-edit-button]',
toggleEditingState);
tasksContainer.on('click','[data-task-edit-abort-button]',
toggleEditingState);
tasksContainer.on('click','[data-task-submit-update-button]',
handleTaskUpdateRequest);
tasksContainer.on('click','[data-task-delete-button]',
